<template>
  <div class="viewarr" @keypress="keyup" tabindex="0">
    <div class="toolbar">
      ...
      <div class="limits">
        <input v-model="vmin" value="min">
        <span>-</span>
        <input v-model="vmax" value="max">
      </div>
...
// Reproduced from https://github.com/matplotlib/matplotlib/blob/584cda0e09fb220bb44f7dc6a3ae53b39c91bb05/lib/matplotlib/_cm_listed.py#L3
// under the terms of the matplotlib license https://github.com/matplotlib/matplotlib/blob/584cda0e09fb220bb44f7dc6a3ae53b39c91bb05/LICENSE/LICENSE#LL1C1-L1C1
// Changes made from matplotlib: the magma colormap data have been extracted into a form usable from JavaScript.

// magma color map raw data is 256 [r, g, b] triples where the components are in [0, 1]
const magmaColormapTable = [[0.001462, 0.000466, 0.013866],
...
// To use, image data must be "normed" somehow to a [0, 1] range
// then multiplied by 256 and the integer part used to find the two closest values.
// Fractional part is then used to linearly interpolate between them.

function norm(value, vmin, vmax) {
  if (value <= vmin) {
    return 0.0;
  } else if (value >= vmax) {
    return 1.0;
...
function cmapAssign(normedValue, lookupTable, destinationArray, destinationBaseIndex) {
  ...
  if (lowIndex == 255) {
    ...
    for (let i = 0; i < 3; i++) {
      destinationArray[destinationBaseIndex + i] = Math.round(255 * ((1 - fractionalPart) * lookupTable[lowIndex][i] + fractionalPart * lookupTable[lowIndex + 1][i]));
    }
  }
}
    ...
    imageData() {
      ...
      for (let y = 0; y < this.height; y++) {
        for (let x = 0; x < this.width; x++) {
          ...
          cmapAssign(norm(this.array[srcIdx], this.vmin, this.vmax), magmaColormapTable, pixels, destIdx);
